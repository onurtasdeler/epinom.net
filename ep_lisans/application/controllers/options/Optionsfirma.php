...
class Optionsfirma  extends CI_Controller
{
    public $formControl = "";
    public $imgId = "";
    public $settings = "";
    public $viewFolder = "firma";
    public $viewFile = "blank";
    public $tables = "opitons_general";
    public $baseLink = "firma-bilgileri";
    ...
    public function index_update()
    {
        ...
        if($_POST){
            ...
            if($_FILES["logo"]["tmp_name"]!=""){
                ...
                $up = img_upload($_FILES["logo"],"firma-logo-".permalink($this->input->post("firmaUnvani")),"logo","","",$ext);
            }

            if($up==""){
                $up=$kontrol->site_logo;
            }

            $guncelle=$this->m_tr_model->updateTable("options_general",array(
                "site_logo" => $up,
                "firma_unvan" => $this->input->post("firmaUnvani"),
                "firma_adres" => $this->input->post("firmaAdresi"),
                "il" => $this->input->post("il"),
                "ilce" => $this->input->post("ilce"),
                "tel" => $this->input->post("firmaTel"),
                "eposta" => $this->input->post("firmaEposta"),
                "vno" => $this->input->post("vergiDairesi"),
                "vdaire" => $this->input->post("vergiNo")
            ),array("id" => 1));
            if($guncelle){
                pageCreate($view, array("hata" => "yok","v" => getTableSingle("options_general",array("id" => 1))), $page, array());
