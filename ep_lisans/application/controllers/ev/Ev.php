...
class Ev extends CI_Controller
{
    ...
    public $imgId = "";
    public $settings = "";
    public $viewFolder = "ev/ev";
    public $viewFile = "blank";
    public $tables = "table_ev_tipleri";
    public $baseLink = "ev_tipleri";
    ...
    public function index()
    {
        ...
        $page = array(
            "pageTitle" => "Ev Tipi Yönetimi - " . $this->settings->firma_unvan,
            "subHeader" => "Ev Tipi Yönetimi",
            "h3" => "Ev Tipi Yönetimi",
            ...
            if ($_POST) {
                ...
                if ($this->input->post("marka_name")) {
                    ...
                    if ($this->input->post("updateId") != "") {
                        //update
                        $kontrol = getTableSingle("table_ev_tipleri", array("id" => $this->input->post("updateId")));
                        if ($kontrol) {
                            $kontrolName = "1";
                                ...
                                if ($_FILES["logo"]["tmp_name"] != "") {
                                    ...
                                    $up = img_upload($_FILES["logo"], "evler-" . permalink($clear), "evler", "", "", $ext);
                                }
                                if ($up == "") {
                                    $up = $kontrol->logo;
                                }
                                $guncelle = $this->m_tr_model->updateTable("table_ev_tipleri", array(
                                    "name" => $clear,
                                    "logo" => $up,
                        ...
                        $kontrol = getTableSingle("table_ev_tipleri", array("name" => $clear));
                        if ($kontrol) {
                            $data = array("err" => true, "message" => $clear . " adında ev tipi mevcut.");
                            echo json_encode($data);
                        } else {
                            if ($_FILES["logo"]["tmp_name"] != "") {
                                $ext = pathinfo($_FILES["logo"]["name"], PATHINFO_EXTENSION);
                                $up = img_upload($_FILES["logo"], "ev-" . permalink($clear), "evler", "", "", $ext);
                            }
    ...
    {
        $toplam = $this->m_tr_model->query("select count(*) as sayi from " . $this->tables . " as s  where is_delete=0 ");
        $sql = "select s.id as ssid,s.name,s.logo,s.status,s.created_at, s.fiyat from " . $this->tables . " as s " ;
        $where = [];
        $order = ['name', 'asc'];
        ...
        if (count($where) > 0) {
            $filter = $this->m_tr_model->query("select count(*) as toplam from " . $this->tables . " as s  " . (count($where) > 0 ? ' WHERE  is_delete=0 and ( ' . implode(' or ', $where) : ' ') . "  ) ");
        } else {
            $filter = $this->m_tr_model->query("select count(*) as toplam from " . $this->tables . " as s where  is_delete=0 ");
        }
