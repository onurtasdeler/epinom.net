...
class Paket extends CI_Controller
{
    ...
    public $imgId           = "";
    public $settings        = "";
    public $viewFolder      = "paket/paket";
    public $viewFile        = "blank";
    public $tables          = "table_paketler";
    public $baseLink        = "paketler";
    ...
    public function index($id="")
    {
        ...
        $page = array(
            "pageTitle" => "Paket Yönetimi - " . $this->settings->firma_unvan,
            "subHeader" => "Paket Yönetimi",
            "h3" => "Paket Yönetimi",
            ...
            if ($_POST) {
                if($this->input->post("marka_name")){
                    ...
                    if($this->input->post("updateId") != ""){
                        ...
                        $kontrol=getTableSingle("table_paketler",array("id" => $this->input->post("updateId")));
                        if($kontrol){
                            if($kontrol->parent==0){
                                $kontrolName="1";
                                if($kontrol->name!=$clear){
                                    $kontrolName=getTableSingle("table_paketler",array("name" => $clear,"parent" => 0 ));
                                    if($kontrolName){
                                        $kontrolName=2;
                                    }
                                }else{
                                ...
                                if($kontrolName=="2"){
                                    $data = array("err" => true, "message" => $clear." adında paket mevcut.");
                                    echo json_encode($data);
                                }else{
                                    $guncelle=$this->m_tr_model->updateTable("table_paketler",array(
                                        "name" => $clear,
                            ...
                            //Ana Hizmet
                            $kontrol=getTableSingle("table_paketler",array("name" => $clear));
                            if($kontrol){
    ...
    public function list_table($id=""){
        ...
        $sql = "select s.id as ssid,s.name,s.status,s.created_at,s.sure,s.fiyat from ".$this->tables." as s ".$join;
        $where = [];
        $order = ['name', 'asc'];
