...
class Odeme extends CI_Controller
{
    ...
    public $imgId = "";
    public $settings = "";
    public $viewFolder = "odemeler";
    public $viewFile = "blank";
    public $tables = "table_odemeler";
    public $baseLink = "odemeler";
    ...
    {
        if($id){
            $cek=getTableSingle("table_musteriler",array("id"  => $id));
            if($cek){
                $view = array(
                ...
                $page = array(
                    "pageTitle" => "Ödeme Yönetimi - " . $this->settings->firma_unvan,
                    "subHeader" => "Ödeme Yönetimi",
                    "h3" => "Ödeme Yönetimi",
                ...
                if($id){
                    ...
                    if($cek){
                        ...
                        if($this->input->post("domain")!=""){
                            ...
                            $ay = date('Y-m-d', strtotime($cek->lisans_bit . ' + '.$paketcek->sure.' months'));
                            $guncelle=$this->m_tr_model->updateTable("table_musteriler",array("lisans_bit" => $ay),array("id" => $cek->id));
    ...
    public function list_table($id = "")
    {
        $join = "";
        if ($id) {
            $w = " s.is_delete=0 and s.li_id =".$id;
        } else {
            $w = " s.is_delete=0 and s.li_id =".$id;
        }
        ...
        foreach ($veriler as $veri) {
            ...
            $response["data"][] = [
                ...
                "tutar" => $veri->tutar,
                "manuel" => $veri->manuel,
                "bas_date" => $veri->bas_date,
                "bit_date" => $veri->bit_date,
