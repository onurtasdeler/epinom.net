...
<script>
    ...
    function singleDelete(id){
        var a="";
        $.post("<?= base_url('get-info') ?>",{id:id,table:"table_ek_hizmetler"},function(response){
            if(response){
    ...
    function singleDeleted(){
        ...
        if(a!=""){
            $.post("<?= base_url('record-delete') ?>",{table:"table_ek_hizmetler",id:a,imageFolder:"marka",imageField:"logo",},function(response){
                if(response){
                    if(response.err){
    ...
    function get_info(id) {
        $.post("<?= base_url("get-info") ?>", {table: "table_ek_hizmetler", id: id}, function (response) {
            if (response) {
    ...
    $(document).ready(function () {
        ...
        $("#markaAddForm").validate({
            ...
            messages: {
                marka_name: "Lütfen Hizmet Başlığını iriniz",
            },
            submitHandler: function (form) {
                var formData = new FormData(document.getElementById("markaAddForm"));
                $.ajax({
                    url: "<?= base_url("ek-hizmet-ekle") ?>",
                    type: 'POST',
                    data: formData,
        ...
        $("#imgDeleted").on("click",function(){
           if($("#updateId").val()){
               $.post("<?= base_url("delete-image") ?>", {table:"table_ek_hizmetler",field:"logo",id:$("#updateId").val(),folder:"hizmetler"}, function (response) {
                   if(response){
                       if(response.err){
                           als(response.message,"error");
                       }else{
        ...
        table = $('#markatable').DataTable({
            ...
            order: [[0, "desc"]],
            ajax: {
                url: "<?= base_url("ek-hizmet-list-table") ?>",
                type: 'POST',
                data: {
                    // parameters for custom backend script demo
                    columnsDef: [
                        'ssid', 'logo','fiyat', 'name', 'created_at', 'status', 'action'],
                },
            },
            "language": {
            ...
            columns: [
                ...
                {data: 'fiyat', responsivePriority: -2,render: $.fn.dataTable.render.number( ',', '.', 2 )},
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    "render": function (data, type, row) {
                        if (row.logo != "") {
                            ...
                            return '<ul class="project-users g-1"><li>' +
                                '<img width="50" height="50" src="<?= base_url("assets/images/a81.png") ?>" alt=""></li></ul>';
                        }
                    }
                }, {
                    orderable: true,
                    ...
                    "render": function (data, type, row) {
                        if (row.status == 1) {
                            return '<div class="custom-control custom-switch">' +
                                '<input type="checkbox" data-url="<?= base_url('ek-hizmet-veri-guncelle/status/') ?>' + row.ssid + '" ' +
...

                {
                    ...
                    render: function (data, type, row) {
                        if(row.tur==1){
                            return '<ul class="nk-tb-actions gx-1"><li><a href="<?= base_url("hizmet/") ?>'+ row.ssid +'" class="btn btn-warning btn-sm"><em class="icon ni ni-list"></em></a></li><li><div class="drodown">' +
