...
<script>
    ...
    function singleDelete(id){
        var a="";
        $.post("<?= base_url('get-info') ?>",{id:id,table:"table_paketler"},function(response){
            if(response){
    ...
    function singleDeleted(){
        ...
        if(a!=""){
            $.post("<?= base_url('record-delete') ?>",{table:"table_paketler",id:a,imageFolder:"",imageField:"",},function(response){
                if(response){
                    if(response.err){
    ...
    function get_info(id) {
        $.post("<?= base_url("get-info") ?>", {table: "table_paketler", id: id}, function (response) {
            if (response) {

                $("#updateId").val(response.id);
                $("#secTitle").html("<b class='text-info'>" + response.name + "</b> - Proje Grubu Güncelle");
                $("#marka_name").val(response.name);
                $("#sure").val(response.sure);
    ...
    $(document).ready(function () {
        ...
        $("#imgDeleted").on("click",function(){
           if($("#updateId").val()){
               $.post("<?= base_url("delete-image") ?>", {table:"table_paketler",field:"logo",id:$("#updateId").val(),folder:"hizmetler"}, function (response) {
                   if(response){
                       if(response.err){
                           als(response.message,"error");
                       }else{
        ...
        table = $('#markatable').DataTable({
            ...
            order: [[0, "desc"]],
            ajax: {
                url: "<?= base_url("paket-list-table") ?>",
                type: 'POST',
                data: {
                    // parameters for custom backend script demo
                    columnsDef: [
                        'ssid', 'name', 'created_at',,'sure','fiyat', 'status', 'action'],
                },
            },
            "language": {
            ...
            columns: [
                ...
                {data: 'sure', responsivePriority: -2},
                    ...
                    "render": function (data, type, row) {
                        if (row.status == 1) {
                            return '<div class="custom-control custom-switch">' +
                                '<input type="checkbox" data-url="<?= base_url('paket-veri-guncelle/status/') ?>' + row.ssid + '" ' +
