...
<script>
    ...
    $(document).ready(function () {
        ...
        table = $('#markatable').DataTable({
            ...
            order: [[0, "desc"]],
            ajax: {
                url: "<?= base_url("musteri-list-table") ?>",
                type: 'POST',
                data: {
                    // parameters for custom backend script demo
                    columnsDef: [
                        'ssid', 'mad', 'plaka','logos', 'marka_adi', 'model_adi',"cdate",'stat','telefon','action'],
                },
            },
            "language": {
            ...
            columns: [
                ...
                {data: 'plaka', responsivePriority: 1},
                {data: 'mad', responsivePriority: 2},
                {data: 'marka_adi', responsivePriority: 2},
                {data: 'model_adi', responsivePriority: 2},
                {data: 'telefon', responsivePriority: 2},
                {data: 'cdate', responsivePriority: 3},
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    "render": function (data, type, row) {
                        if (row.logos != "") {
                            return "<img src='<?= base_url("upload/marka/")?>" + row.logos +"' width='20' height='20' > " +row.marka_adi;
                        } else {
                    ...
                    "render": function (data, type, row) {
                        if (row.stat == 1) {
                            return '<div class="custom-control custom-switch">' +
                                '<input type="checkbox" data-url="<?= base_url('musteri-veri-guncelle/status/') ?>' + row.ssid + '" ' +
...

                {
                    ...
                    render: function (data, type, row) {
                        return '<ul class="nk-tb-actions gx-1"><li><div class="drodown">' +
                            ...
                            '<li><a href="<?= base_url("musteriler/") ?>' + row.ssid + '" ><em class="icon ni ni-search"></em><span>Cari Detayları</span></a></li>' +
