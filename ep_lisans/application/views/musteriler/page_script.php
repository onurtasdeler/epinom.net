...
<script>
    ...
    $(document).ready(function () {
        ...
        table = $('#markatable').DataTable({
            ...
            order: [[0, "desc"]],
            ajax: {
                url: "<?= base_url("lisans-list-table") ?>",
                type: 'POST',
                data: {
                    // parameters for custom backend script demo
                    columnsDef: [
                        'ssid', 'ad_soyad','lisans_bas','lisans_bit','lisans_domain','proje_adi', 'created_at', 'stat', 'action'],
                },
            },
            "language": {
                    ...
                    "render": function (data, type, row) {
                        if (row.stat == 1) {
                            return '<div class="custom-control custom-switch">' +
                                '<input type="checkbox" data-url="<?= base_url('lisans-veri-guncelle/status/') ?>' + row.ssid + '" ' +
