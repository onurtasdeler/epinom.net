...
<script>
    ...
    function singleDeleted(){
        ...
        if(a!=""){
            $.post("<?= base_url('record-delete') ?>",{table:"table_hizmetler",id:a,imageFolder:"hizmetler",imageField:"logo",},function(response){
                if(response){
                    if(response.err){
    ...
    $(document).ready(function () {
        ...
        $("#formBackButton").on("click",function(){
            ...
            $("#secTitle").html("Proje Grubu Ekle");
            $("#updateId").val();
        ...
        table = $('#markatable').DataTable({
            ...
            order: [[0, "desc"]],
            ajax: {
                url: "<?= base_url("saldiri-list-table") ?>",
                type: 'POST',
                data: {
                    // parameters for custom backend script demo
                    columnsDef: [
                        'ssid', 'ip','domain','agent', 'created_at', 'status', 'action'],
                },
            },
            "language": {
            ...
            columns: [
                ...
                {data: 'ip', responsivePriority: -2},
                {data: 'domain', responsivePriority: -2},
                {data: 'agent', responsivePriority: -2},
