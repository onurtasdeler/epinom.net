...
<script>
    function sosyalDelete(id){
        var a="";
        $.post("<?= base_url('urun-stok-cek') ?>",{data:id},function(response){
    ...
    function sosyal_delete(){
        var a=$("#silinecek").val();
        if(a!=""){
            $.post("<?= base_url('urun-stok-sil') ?>",{data:a},function(response){
                if(response){
                    alertToggle(1,"Stok Kodu Silindi.","İşlem Başarılı");
