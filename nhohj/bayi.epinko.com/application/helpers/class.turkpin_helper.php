...
class TurkPin
{
    ...
    function siparisVer($oyun, $urun, $adet, $character = NULL)
    {
        ...
        $cek=$t->m_tr_model->getTableSingle("table_options_sms",array("id" => 1));
        if($cek->modul_aktif_turkpin==1 && $cek->turkpin_username!="" && $cek->turkpin_password!=""){
            $xml = '<?xml version="1.0" encoding="utf-8"?>
            <APIRequest>
                <params>
                    <cmd>epinSiparisYarat</cmd>
                    <username>' . $cek->turkpin_username . '</username>
                    <password>' . $cek->turkpin_password . '</password>
