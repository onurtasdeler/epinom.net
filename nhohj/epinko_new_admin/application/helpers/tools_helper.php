    ...
    function upload_picture($file,$viewFolder,$name,$webp=""){
        ...
        try {
            ...
            if ($image->uploaded) {
                ...
                $image->allowed = array('image/jpg','image/gif', 'image/jpeg', 'image/png','image/ico','image/svg'); //izin veirlen uzantılar
...
function upload_picture_category($file,$viewFolder,$name,$webp=""){
    ...
    try {
        ...
        if ($image->uploaded) {
            ...
            $ayar=$t->m_tr_model->getTableSingle("options_general",array("id" => 1));

            $watermark_file = $ayar->site_link."/upload/logo/".$ayar->site_logo;
           ...
           // $image->image_watermark_position = 'C';
...

{
    ...
    function get_api_code($data){
        if ($data->pinabi_id > 0){
            return 'pinabi';
        }elseif ($data->turkpin_id > 0){
            return 'turkpin';
        }
    }
