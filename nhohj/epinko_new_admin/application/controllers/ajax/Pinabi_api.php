...
class Pinabi_api extends CI_Controller
{
        ...
        if ($this->formControl == $this->session->userdata("formCheck")) {
            if ($_POST) {
                if ($this->input->post("data")) {
                    $pinabi = new Pinabi();
                    $sonuclar = $pinabi->urunListesi($this->input->post("data"));
                    ?>
                    <label><b>Pinabi </b>Ürün</label>
                    <select name="pinabi_urun_id" class="form-control pinabioyun">
    ...
    public function get_product_price()
    {
        if ($this->formControl == $this->session->userdata("formCheck")) {
            if ($_POST) {
                ...
                $pinabi = new PinAbi();
                $sonuclar = $pinabi->urunListesi($this->input->post("cat"));
                foreach($sonuclar as $row)
                {
    ...
    public function get_category_product_2()
    {
        if ($this->formControl == $this->session->userdata("formCheck")) {
            if ($_POST) {
                if ($this->input->post("data")) {
                    $pinabi =new PinAbi();
                    $oyunlar = $pinabi->getAllCategoriesAndProducts()['gameList'];
					...
					foreach ($oyunlar as $oyun){
						if ($oyun['id'] == $this->input->post("data")){
							foreach($oyun['productList'] as $row){
								$vericek=$this->m_tr_model->getTableSingle("table_import_pinabi_product",array("pinabi_cat_id" => $this->input->post("data"),"pinabi_urun_id" => $row['id']));
								if($vericek){
									if($vericek->status==1){
