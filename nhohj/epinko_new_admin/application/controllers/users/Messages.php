...
class Messages extends CI_Controller
{
    ...
    public $imgId = "";
    public $settings = "";
    public $viewFolder="users/messages";
    public $viewFile="blank";
    public $tables="table_users_message_gr";
    public $baseLink="uye-mesajlar";
    ...
    public function index_sohbetler()
    {



                $view = array("viewFile" => $this->viewFile, "viewFolder" => "users/messages/list", "viewFolderSafe" => "adverts_messages");
                $page = array(
                    "pageTitle" => "Üye Sohbet Yönetimi - " . $this->settings->site_name,
                    "subHeader" => "Üye-Mağaza Sohbet Yönetimi - <a href='" . base_url("uye-mesajlar") . "'>Kullanıcı Sohbetleri</a>",
                    "h3" => "Kullanıcı Mesajlaşmaları",
                    "btnText" => "", "btnLink" => base_url("ilan-ekle"));
    ...
    public function list_table_message(){
        $toplam = $this->m_tr_model->query("select count(*) as sayi from table_users_message_gr as s ");
        $sql = "select s.id as ssid,s.created_at as cdate,sel.magaza_name as sn,buy.nick_name as bn from table_users_message_gr as s  ";
        $join=" left join table_users as sel on s.seller_id=sel.id left join table_users as buy on s.user_id=buy.id  ";
        $where = [];
        $order = ['cdate', 'asc'];
        $column = $_POST['order'][0]['column'];
        $columnName = $_POST['columns'][$column]['data'];
        ...
        $response["recordsTotal"] = $toplam[0]->sayi;
        if(count($where)>0){
            $filter = $this->m_tr_model->query("select count(*) as toplam from table_users_message_gr as s   ".$join ." ". (count($where) > 0 ? ' WHERE  ( '. implode(' or ', $where) : ' ')." ) ");
        }else{
            $filter = $this->m_tr_model->query("select count(*) as toplam from table_users_message_gr as s  ".$join."  ");
        }
        $response["recordsFiltered"] = $filter[0]->toplam;
        foreach ($veriler as $veri) {
            $response["data"][] = [
    ...
    public function get_record()
    {
        if ($this->formControl == $this->session->userdata("formCheck")) {
            if ($_POST) {
                $veri = getRecord($this->tables, "islemNo", $this->input->post("data"));
                echo $veri;
            } else {
                redirect(base_url("404"));
            }
        ...
        if ($this->formControl == $this->session->userdata("formCheck")) {
            if ($_POST) {
                ...
                if ($this->input->post("data")) {
                    ...
                    if ($kontrol) {
                        $gunell=$this->m_tr_model->delete($this->tables,array("id" => $kontrol->id));
                        $gunell=$this->m_tr_model->delete("table_users_message",array("gr_id" => $kontrol->id));
