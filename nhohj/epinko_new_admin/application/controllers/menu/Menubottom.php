...
class Menubottom extends CI_Controller
{
    public $formControl = "";
    public $imgId = "";
    public $settings = "";
    public $viewFolder="menu_items/bottom";
    public $viewFile="blank";
    public $tables="table_menus";
    public $baseLink="alt-menu-ayarlari";
    public $titleAdTekil="Alt Menü";
    public $titleAdCogul="Alt Menüler";
    public $titleAd="Alt Menüler";
    public $nameTekil="alt-menu";
    public $nameCogul="alt-menuler";
    public $addlink="ekle";
    ...
    public function list_table(){
        $toplam = $this->m_tr_model->query("select count(*) as sayi from ".$this->tables." as s   where (s.is_delete=0 and s.tip=2 )   ");
        $sql = "select s.id as ssid,s.name,s.ikon,s.status ,s.order_id,s.parent from ".$this->tables." as s ";
        $where = [];
        $order = ['order_id', 'asc'];
        ...
        if (count($where) > 0) {
            $sql .= ' WHERE (s.is_delete=0 and s.tip=2 ) and (  '. implode(' or ', $where)." ) ";
            $sql .= " order by " . $order[0] . " " . $order[1] . " ";
            $sql .= " LIMIT " . $_POST['start'] . ", " . $_POST['length'];
        } else {
            $sql .= " where (s.is_delete=0 and s.tip=2 ) order by " . $order[0] . " " . $order[1] . " ";
            $sql .= " LIMIT " . $_POST['start'] . ", " . $_POST['length'];
        }
        ...
        if(count($where)>0){
            $filter = $this->m_tr_model->query("select count(*) as toplam from ".$this->tables." as s  " . (count($where) > 0 ? ' WHERE (s.tip=2 and s.is_delete=0) and  ( ' . implode(' or ', $where) : ' ')." )  ");
        }else{
            $filter = $this->m_tr_model->query("select count(*) as toplam from ".$this->tables." as s "." where (s.is_delete=0 and s.tip=2 ) ");
        }
        ...
        if ($_POST) {
            if ($this->formControl == $this->session->userdata("formCheck")) {
                if ($veri["err"] != true) {
                    ...
                    $order=getLastOrder("table_menus"," where parent=".$this->input->post("ustmenu")." and tip=2 and is_delete=0" );
                ...
                if ($this->formControl == $this->session->userdata("formCheck")) {
                    if ($veri["err"] != true) {
                        ...
                        if($this->input->post("order_id")!=$kontrol->order_id){
                            $cekYeni=getTableSingle("table_menus",array("order_id" => $this->input->post("order_id"),"parent" => $kontrol->parent,"tip" => 2,"is_delete" => 0));
                            if($cekYeni){
    ...
    public function get_record_all_delete()
    {
        if ($this->formControl == $this->session->userdata("formCheck")) {
            ...
            if($this->input->post("tur")==1){
                ...
                $str=rtrim($str,",");
                $cek =$this->m_tr_model->query("select * from ".$this->tables." where is_delete=0 and tip=2 and id in(".$str.") ");
                if($cek){
                    foreach ($cek as $item) {
