...
<script>
    function sosyalDelete(id){
        var a="";
        $.post("<?= base_url('anasayfa-urun-cek') ?>",{data:id},function(response){
    ...
    function sosyal_delete(){
        var a=$("#silinecek").val();
        if(a!=""){
            $.post("<?= base_url('anasayfa-urun-sil') ?>",{data:a},function(response){
                if(response){
                    alertToggle(1,"Anasayfa Ürün Kategorisi Silindi.","İşlem Başarılı");
