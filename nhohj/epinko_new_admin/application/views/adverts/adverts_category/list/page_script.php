...
<script>
    function sosyalDelete(id){
        var a="";
        $.post("<?= base_url('ilan-kategori-cek') ?>",{data:id},function(response){
    ...
    function sosyal_delete(){
        var a=$("#silinecek").val();
        if(a!=""){
            $.post("<?= base_url('ilan-kategori-sil') ?>",{data:a},function(response){
                if(response){
                    alertToggle(1,"İlan Kategorisi ve Alt Kategorileri Silindi.","İşlem Başarılı");
