...
<script>
    function sosyalDelete(id){
        var a="";
        $.post("<?= base_url('ilan-doping-fiyatlar-cek') ?>",{data:id},function(response){
    ...
    function deleteModalSubmit(){
        ...
        var tur=$("#tur").val();
        if(a!=""){
            $.post("<?= base_url('ilan-gorsel-img-sil') ?>",{data:a,tur:tur},function(response){
                if(response){
    ...
    function sosyal_delete(){
        var a=$("#silinecek").val();
        if(a!=""){
            $.post("<?= base_url('ilan-doping-fiyatlar-sil') ?>",{data:a},function(response){
                if(response){
                    alertToggle(1,"İlan Görsel Silindi.","İşlem Başarılı");
