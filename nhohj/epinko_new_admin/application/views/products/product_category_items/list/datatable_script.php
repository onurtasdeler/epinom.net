        ...
        var KTDatatablesDataSourceAjaxServer = function() {
            ...
            var initTable1 = function() {
                ...
                table.DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: "<?= base_url("category-list-table".$w) ?>",
                        type: 'POST',
                        data: {
                            // parameters for custom backend script demo
                            columnsDef: [
                                'ssid','order_id','c_name','image','status','bayi',"parent","parent_id",'bayi_komisyon','bayi_link','is_new','is_populer','is_anasayfa','action'],
                        },
                    },
                    "language":{
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            "render": function (data, type, row) {
                                if(row.bayi==2){
                                   return row.c_name;
                                    ...
                                    return row.c_name + "<br> BAYİ: <a href='<?= base_url("uye-guncelle") ?>/"+ row.bayi_link +"'>" + row.bayi + " - Komisyon: %" + row.bayi_komisyon + "</a>" ;
                                }
                            }
                        },{
                            orderable:true,
                            ...
                            "render": function (data, type, row) {
                                if(row.parent_id!=0){
                                    ...
                                    return '<span class="badge badge-info">Ana Kategori</span>';
                                }
                        ...
                        },
                        {
                            ...
                            targets:6,
                            "render": function (data, type, row) {
                                if(row.is_show_tab==1){
                                    return '<span class="switch switch-outline switch-icon switch-success">' +
                                        '<label><input type="checkbox" id="switch-lg_6_'+ row.ssid +'" checked data-url="<?= base_url('kategori-veri-guncelle/is_show_tab/') ?>' + row.ssid + '" ' +
                                        'onchange="durum_degistir(6,'+ row.ssid +')" name="select"><span></span></label></span>';
                                }else{
                                    return '<span class="switch switch-outline switch-icon switch-success">' +
                                        '<label><input type="checkbox" id="switch-lg_6_'+ row.ssid +'"  data-url="<?= base_url('kategori-veri-guncelle/is_show_tab/') ?>' + row.ssid + '" ' +
                                        'onchange="durum_degistir(6,'+ row.ssid +')" name="select"><span></span></label></span>';
                                }
                            }
                        },
                            ...
                            render: function(data, type,row) {
                                ...
                                if(row.parent_id!=0){
                                    ...
                                    return '<a href="<?= base_url('kategoriler?down=') ?>' + row.ssid + '" class="btn btn-sm btn-clean btn-icon" title="Aşağı Taşı">'+
                                        ...
                                        '<a href="<?= base_url('kategoriler?up=') ?>'+ row.ssid +'" class="btn btn-sm btn-clean btn-icon" title="Yukarı Taşı">' +
                                        '<i class="la la-arrow-up text-primary"></i></a>' +
                                        '<a href="<?= base_url('kategoriler?sub=') ?>' + row.ssid + '" class="btn btn-sm btn-clean btn-icon" title="">'+
                                        '<i class="la la-list text-warning"></i></a>' +
