...
<script>
    ...
    $(document).ready(function () {
        ...
        // category_id değiştiğinde
        $('#category_id').change(function () {
            var selectedCategoryId = $(this).val();

            var selectedCategories = $('#related_categories').val();

            // Eğer eski category_id seçili ise, onu kaldıralım
            if (selectedCategories && selectedCategories.indexOf(selectedCategoryId) !== -1) {
                selectedCategories.splice(selectedCategories.indexOf(selectedCategoryId), 1);
                $('#related_categories').val(selectedCategories).trigger('change'); // Seçili öğeyi kaldır
            }

            // Tüm seçenekleri önce etkinleştir
            $('#related_categories option').prop('disabled', false);

            // Eğer bir değer seçilmişse, categories içinde devre dışı bırak
            if (selectedCategoryId) {
                $('#related_categories option[value="' + selectedCategoryId + '"]').prop('disabled', true);
            }
        ...
        var ilkval="";
        $("#turkpin_auto_order").change(function () {
            if ($(this).val() == 0) {
                $("#turkpin_auto_stock").val("0");
                $("#turkpin_stok").val(0);
            }
        });

        $("#turkpin_auto_stock").change(function () {
            if(ilkval==""){

                    ilkval=$("#turkpin_stok").val();
            ...
            if ($(this).val() == 0) {
                ...
                $("#turkpin_stok").val(ilkval);
                if ($("#turkpin_auto_order").val() == 0) {
                    $("#turkpin_auto_stock").val("0");
                } else {
                    $("#turkpin_auto_stock").val();
                }
            }
        ...
        $("#is_stock").change(function () {
            if ($(this).is(':checked')) {
                if ($("#is_api").is(':checked')) {
                    ...
                    toastr.warning("Lütfen API Bağlantısını kapatınız..");
                } else {
        ...
        $("#is_api").change(function () {
            ...
            if ($(this).is(':checked')) {
                $("#card_customm").css("background-color", "#56c5c0");
                $("#apiBaslik").html("API Ayarları (AKTİF)");
                if ($("#is_stock").is(':checked')) {
                    $("#is_stock").prop('checked', false);
                } else {
                    //$("#card_customm").css("background-color","#cb5353");
                }
            } else {
                $(".turkpinkategori").val("0");
                $(".turkpinoyun").val("0");
                $("#card_customm").css("background-color", "#cb5353");
                $("#apiBaslik").html("API Ayarları (PASİF)");
            }
        });
