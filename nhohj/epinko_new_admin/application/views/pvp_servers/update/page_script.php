...
<script>
    ...
    function deleteModal(id,tur){
        var a="";
        $.post("<?= base_url('pvp-server-cek') ?>",{data:id},function(response){
            if(response){
    ...
    function deleteModalSubmit(){
        ...
        var tur=$("#tur").val();
        if(a!=""){
            $.post("<?= base_url('pvp-server-img-sil') ?>",{data:a,tur:tur},function(response){
                if(response){
    ...
    $(document).ready(function() {
        ...
        $("#guncelleForm").on("submit",function (e){
            ...
            $.ajax({
                url:"<?= base_url("pvp-server-guncelle/".$data["veri"]->id) ?>",
                type: 'POST',
                data: formData,
