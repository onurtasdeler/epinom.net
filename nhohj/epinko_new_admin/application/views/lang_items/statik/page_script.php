...
<script>
    ...
    function langStatik(id){
        var tr=$("#tr_" + id).val();
        var en=$("#en_" + id).val();
        $.post("<?= base_url('lang-statik-update') ?>",{data:id,en:en,tr:tr},function(response){
            if(response){
                alertToggle(1,"Statik veri başarılı şekilde güncellendi.","İşlem Başarılı");
                //setTimeout(function(){ window.location.reload(); }, 400);
            }else{
                alertToggle(2,"Bir hata meydana geldi.","Hata");
            }
