...
<script>
    function sosyalDelete(id){
        var a="";
        $.post("<?= base_url('rezervasyon-cek') ?>",{data:id},function(response){
    ...
    function sosyal_delete(){
        var a=$("#silinecek").val();
        if(a!=""){
            $.post("<?= base_url('rezervasyon-sil') ?>",{data:a},function(response){
                if(response){
                    alertToggle(1,"Rezervasyon Silindi.","İşlem Başarılı");
