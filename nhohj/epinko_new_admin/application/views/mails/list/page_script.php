...
<script>
    ...
    function sosyalDelete(id){
        var a="";
        $.post("<?= base_url('mails/mails/get_record') ?>",{data:id},function(response){
    ...
    $(document).ready(function() {
        ...
        $('#sablon').select2({
            ...
            $.ajax({
                url:  "<?= base_url("mails/mails/sablon_getir") ?>",
                type: 'POST',
                ...
                success: function (response) {
                    if(response){
                        ...
                        $("#mailkonu").val(response.konu);
                        tinymce.get('editor1').setContent(response.icerik);
                    }else{
                        return false;
                    }
                },
        ...
        $('#grup').select2({
            ...
            tags: true,
            ajax: {
                url: '<?= base_url("mails/mails/getusertop") ?>', // Verilerinizi çeken PHP dosyasının URL'si
                dataType: 'json',
        ...
        $('#kategori').select2({
            ...
            tags: true,
            ajax: {
                url: '<?= base_url("mails/mails/getusercategory") ?>', // Verilerinizi çeken PHP dosyasının URL'si
                dataType: 'json',
                data: function (params) {
                    return {
                        q: params.term // Arama sorgusu
        ...
        $("#sablonKaydetButton").on("click",function (){
            var veri=tinymce.get('editor1').getContent();
            if($("#mailkonu").val() && veri!=""){
                $.ajax({
                    url:  "<?= base_url("mails/mails/sablon_kaydet") ?>",
                    type: 'POST',
                    data: {name:$("#mailadi").val(),konu:$("#mailkonu").val(),icerik:tinymce.get('editor1').getContent()},
                    success: function (response) {
        ...
        $("#guncelleForm").on("submit",function (){
            if($("#kategori").val()!=""){
                ...
                fdata.append('micerik', veri);
                $.ajax({
                    url:"<?= base_url() ?>mails/mails/send_mails",
                    type: 'POST',
                    data:fdata,
                    success: function (response) {
                        if(response.hata=="var"){
