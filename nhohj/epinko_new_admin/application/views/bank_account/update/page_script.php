...
<script>
    $(function(){
        ...
        ?>
        $ck<?= $item->id ?> = CKEDITOR.replace('icerik1_<?= $item->id ?>', {
...
function deleteModal(id){
        var a="";
        $.post("<?= base_url('banka-hesap-cek') ?>",{data:id},function(response){
            if(response){
    ...
    function deleteModalSubmit(){
        ...
        var tur=1;
        if(a!=""){
            $.post("<?= base_url('banka-hesap-img-sil') ?>",{data:a,tur:tur},function(response){
                if(response){
    ...
    $(document).ready(function() {
        ...
        $("#guncelleForm").on("submit",function (e){
            ...
            $.ajax({
                url:"<?= base_url("banka-hesap-guncelle/".$data["veri"]->id) ?>",
                type: 'POST',
                data: formData,
