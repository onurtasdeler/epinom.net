...
<script>
    ...
    function deleteModal(id,tur){
        var a="";
        $.post("<?= base_url('slider-cek') ?>",{data:id},function(response){
            if(response){
    ...
    function deleteModalSubmit(){
        ...
        var tur=$("#tur").val();
        if(a!=""){
            $.post("<?= base_url('slider-img-sil') ?>",{data:a,tur:tur},function(response){
                if(response){
    ...
    $(document).ready(function() {
        ...
        $("#guncelleForm").on("submit",function (e){
            ...
            $.ajax({
                url:"<?= base_url("admin-kullanicilar-guncelle/".$data["veri"]->id) ?>",
                type: 'POST',
                data: formData,
