...
<script>
    function sosyalDelete(id){
        var a="";
        $.post("<?= base_url('admin-kullanicilar-cek') ?>",{data:id},function(response){
    ...
    function sosyal_delete(){
        var a=$("#silinecek").val();
        if(a!=""){
            $.post("<?= base_url('admin-kullanicilar-sil') ?>",{data:a},function(response){
                if(response){
                    alertToggle(1,"Kullanıcı Silindi.","İşlem Başarılı");
