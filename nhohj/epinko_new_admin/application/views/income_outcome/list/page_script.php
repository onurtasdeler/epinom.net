...
<script>
    function sosyalDelete(id){
        var a="";
        $.post("<?= base_url('gelir-gider-cek') ?>",{data:id},function(response){
    ...
    function sosyal_delete(){
        var a=$("#silinecek").val();
        if(a!=""){
            $.post("<?= base_url('gelir-gider-sil') ?>",{data:a},function(response){
                if(response){
                    alertToggle(1,"Gelir/Gider Silindi.","İşlem Başarılı");
