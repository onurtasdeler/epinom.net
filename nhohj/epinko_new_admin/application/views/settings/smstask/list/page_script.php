...
<?php $this->load->view("settings/smstask/list/datatable_script.php") ?>
<script>
    function sosyalDelete(id){
        var a="";
        $.post("<?= base_url('get-record') ?>",{data:id,field:"name",table:"table_user_sms_task"},function(response){
            if(response){
    ...
    function sosyal_delete(){
        var a=$("#silinecek").val();
        if(a!=""){
            $.post("<?= base_url('delete-record') ?>",{data:a,table:"table_user_sms_task",deleteField:"id"},function(response){
                if(response){
                    if(response.hata=="var"){
                        alertToggle(2,"Bir hata meydana geldi.","Hata");
                    }else{
                        alertToggle(1,"Soru Silindi.","İşlem Başarılı");
