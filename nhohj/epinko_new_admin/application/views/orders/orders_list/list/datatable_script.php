                ...
                table.DataTable({
                    ...
                    ajax: {
                        ...
                        data: {
                            ...
                            columnsDef: [
                                'ssid','full_name',"sipNo",'nick_name',"price_discount","product_id",'image_urun','cat_name','quantity','price','user_id','is_raffle','is_case','status','cdate','invoice_id','invoice_date','action'],
                        },
                    },
                    "language":{
                    ...
                    columnDefs: [
                        ...
                        },*/
                        {
                            ...
                            "render": function (data, type, row) {
                                if(row.price_discount!=0){
                                    return row.total_price + " <?= getcur() ?> <br> <small class='text-info'> İndirimli </small>       ";
                                }else{
                        ...
                        },
                        {
                            ...
                            "render": function (data, type, row) {
                                if(row.is_raffle==1){
                                    return '<span style="height:auto;" class="label label-inline label-light-warning font-weight-bold">Çekiliş Ödülü</span>';
                                } else if(row.is_case==1) {
                                    return '<span style="height:auto;" class="label label-inline label-light-danger font-weight-bold">Kasa Ödülü</span>';
                                } else {
                                    return '<span style="height:auto;" class="label label-inline label-light-primary font-weight-bold">Normal Sipariş</span>';
                                }
                            }
                        },
                        {
                            ...
                            "render": function (data, type, row) {
                                if(row.invoice_id==""){
                                    return '<a href="<?= base_url("siparis-faturalandir/") ?>' + row.ssid +'" class="btn btn-sm btn-danger" />Faturala</a>';
                                }else{
                                    return '<a target="_blank" class="btn btn-sm btn-success" style="" href="<?= base_url("fatura-goruntule/") ?>' + row.invoice_id +'" />Faturalandı</a>';
                                }
                            }
                        }, {
                            orderable:true,
