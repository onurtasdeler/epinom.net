...
<script>
    ...
    $(document).ready(function() {
        ...
        var tik=2;
        $("#otocek").on("click",function(){
            if(tik==2){
                $("#cekConfirm").fadeIn(300);
                tik=1;
            }else{

            }
        });
        $("#kodCekOk").on("click",function(){
            if(tik==1){
                $("#kodCekOk").prop("disabled",true);
                alertS("Lütfen Bekleyiniz..","warning");
                $.ajax({
                    url:"<?= base_url("kod-cek-turkpin/".$data["veri"]->id) ?>",
                    type: 'POST',
                    data: {id:<?= $data["veri"]->id ?>},
