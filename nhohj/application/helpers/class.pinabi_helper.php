...
class PinAbi
{
    ...
    function getBalance($currency = NULL)
    {
        $t=&get_instance();
        $t->load->model("m_tr_model");
        $cek=$t->m_tr_model->getTableSingle("table_integration",array("id" => 2));
        if($cek->status==1 && $cek->username!="" && $cek->password!=""){
			...
			if ($currency){
				foreach(json_decode($result, true)['balances'] as $cur){
	...
	function urunListesi($category_id){
		$oyunlar = $this->getAllCategoriesAndProducts();
		foreach($oyunlar['gameList'] as $oyun){
			if ($oyun['id'] == $category_id){
				return $oyun['productList'];
			}
		}
	}
    ...
    function siparisVer($oyun, $urun, $adet, $character = NULL){
        ...
        $cek=$t->m_tr_model->getTableSingle("table_integration",array("id" => 2));
        if($cek->status==1 && $cek->username!="" && $cek->password!=""){
            $result = $this->send('/setOrder', $this->get_auth_data(), json_encode([
                'transactionId' =>  md5($oyun.$urun.time().rand(1,9999)),
                'quantity'  =>  $adet,
                'productId' =>  $urun
            ]), true);
            return json_decode($result, true);
