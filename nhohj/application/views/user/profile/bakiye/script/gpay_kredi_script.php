<script>
    ...
    table.DataTable({
        ...
        order: [[1, "desc"]],
        ajax: {
            url: "<?= base_url("get-payment-list/gpay") ?>",
            type: 'POST',
            data: {
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    if(row.durum==0){
                        ...
                        return '<b class="text-success"><i class="fa fa-check "></i> Ödendi <a href="javascript:;" onclick="modalGoster(\'' + row.description + '\')"><i class="fa fa-info-circle text-success p-1"></i></a></b>';
                    }else if(row.durum==2){
                        return '<b class="text-danger"><i class="fa fa-times "></i> Başarısız <a href="javascript:;" onclick="modalGoster(\'' + row.description + '\')"><i class="fa fa-question-circle text-warning"></i></a> </b>';
                    }else if(row.durum==4){
                        return '<b class="text-danger"><i class="fa fa-times "></i> Başarısız <a href="javascript:;" onclick="modalGoster(\'' + row.description + '\')"><i class="fa fa-question-circle text-warning"></i></a> </b>';
                    }
                }
    ...
    $gpay = getTableSingle("table_payment_methods", array("id" => 5));
    if($gpay->status==1){
    ?>
    $('#priceGpayCredi').on('input', function() {
        var input = $(this);
        var value = input.val();
    ...
    $("#priceGpayCredi").on("input keyup change",function (){
        var vals=$(this).val();
    ...
    $("#btnGpayCredit").on("click",function (){
        $("#btnGpayCredit").prop("disabled",true);
        var vals=$("#priceGpayCredi").val();
        if(vals && vals>0 && vals<10000000){
            $.ajax({
                url:"<?= base_url() ?>get-gpay-credit",
                type: 'POST',
                data: {price:vals,lang:"<?= $_SESSION["lang"] ?>"},
                success: function (response) {
                    if(response){
                        if(response.hata=="var"){
                            $("#btnGpayCredit").prop("disabled",false);
                            if(response.type){
                                window.location.reload();
                            }else{
                                toastr.warning(response.message);
