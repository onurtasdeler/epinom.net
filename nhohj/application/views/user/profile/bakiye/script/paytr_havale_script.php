<script>
    ...
    table.DataTable({
        ...
        order: [[2, "desc"]],
        ajax: {
            url: "<?= base_url("get-payment-list/paytr-havale") ?>",
            type: 'POST',
            data: {
        ...
        columnDefs: [
            {
                ...
                "render": function (data, type, row) {
                    return " <img style=' width:50px;' src='<?= base_url("assets/images/paytr-logo.png") ?>'>";
                }
            },
            {
                orderable: false,
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    if(row.durum==0){
                        ...
                        return '<b class="text-danger"><i class="fa fa-times "></i> Başarısız <br> <small>' + row.description + '</small></b>';
                    }else if(row.durum==4){
                        return '<b class="text-danger"><i class="fa fa-times "></i> Başarısız - <br> <small>' + row.description + '</small> </b>';
                    }
                }
