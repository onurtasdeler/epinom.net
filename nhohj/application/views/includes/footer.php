<div style="padding-top: 30px" class="nu-subscribe-area rn-section-gapTop sal-animate" data-sal-delay="200"
    ...
    <div class="container">
        <div class="row">
            <p><?= substr($this->setting->anasayfa_yazi, 0, 800); ?> <?php if (strlen($this->setting->anasayfa_yazi) > 800): ?><b onclick="document.querySelector('#loadMore').style.display = 'block';this.style.display='none'" style="cursor:pointer;">Devamını Göster</b><?php endif; ?><span id="loadMore" style="display: none;"><?= substr($this->setting->anasayfa_yazi,  800, 9999999) ?></span></p>
        </div>
    </div>
</div>
...
<div class="rn-footer-one rn-section-gap bg-color--1 mt--100 mt_md--80 mt_sm--80 ">
    <div class="container">
        <div class="row gx-5">
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12 mt_md--40 mt_sm--40">
                <div class="footer-widget widget-information">
                    <h6 style="font-size:18px" class="widget-title"><?= ($_SESSION["lang"] == 1) ? "Ürünler" : "Products" ?></h6>
                    <ul class="footer-list-one">
                        <?php
                        $tum = getLangValue(34, "table_pages");
                        $blog = getLangValue(33, "table_pages");
                        ...
                        function findParent($ref, $category_id){
                            $categories = $ref->m_tr_model->query("select * from table_products_category where id = " . $category_id);
                            foreach($categories as $category){
                                if ($category->parent_id){
                                    return findParent($ref, $category->parent_id);
                                }else{
                                    return $category->seflink_tr;
                                }
                            }
                        }


                        $randCat = $this->m_tr_model->query("select * from table_products where status=1 and (turkpin_stok>0 or stok>0) order by rand() limit 7");
                        if ($randCat) {
                            foreach ($randCat as $item) {
                                $cat = findParent($this, $item->category_id);
                                $url = base_url(gg() . $detay->link . "/" . $cat . "/" . ($_SESSION["lang"] == 1 ? $item->p_seflink_tr : $item->p_seflink_en ) );
                        ?>
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12 mt_md--40 mt_sm--40">
                <div class="">
                    <h6 style="font-size:18px" class="widget-title">Sayfalarımız</h6>
                    <ul class="footer-list-one">
                        <?php

                        $kruumsl = getTableOrder("table_menus", array("tip" => 1, "status" => 1, "parent" => 0), "order_id", "asc");
                        if ($kruumsl) {
                            foreach ($kruumsl as $item) {
                                $ll = getLangValue($item->id, "table_menus");
                        ?>
        ...
        <hr class="mt-4" />
        <h6 style="font-size:14px" class="widget-title">İletişim</h6>
        <div class="row">
            <div class="col-lg-2 col-md-6 col-sm-6 col-12 mb-3">
                <div class="">
                    <b>Vergi Dairesi / Numarası</b><br />
                    <?= $this->iletisim->taxOffice ?>
                </div>
            </div>
            <div class="col-lg-2 col-md-6 col-sm-6 col-12 mb-3">
                <div class="">
            ...
            <div class="col-lg-2 col-md-6 col-sm-6 col-12 mb-3">
                <div class="">
                    <b>Adres</b><br />
                    <?= $this->iletisim->address ?>
                </div>
            </div>
            <div class="col-lg-2 col-md-6 col-sm-6 col-12 mb-3">
                <div class="">
                    <b>Müşteri Temsilcisi</b><br>
                    <a href="tel:<?= $this->iletisim->telefon ?>">
                        <?= $this->iletisim->telefon ?>
                    </a>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 col-12 mb-5">
                <div class="copy-right-one">
                    <b>Sosyal Medya</b><br>
                    <div class="copyright-right mt-3">
...
<div class="copy-right-one ptb--20 bg-color--2 ">
    <div class="container">
...
<?php if($this->iletisim->wp_status == 1): ?>
<script>
var wa_btnSetting = {"btnColor":"#16BE45","ctaText":"Whatsapp","cornerRadius":40,"marginBottom":20,"marginLeft":20,"marginRight":20,"btnPosition":"right","whatsAppNumber":"<?= trim(str_replace(' ','',$this->iletisim->wp),'+') ?>","welcomeMessage":"","zIndex":999999,"btnColorScheme":"light"};
window.onload = () => {
