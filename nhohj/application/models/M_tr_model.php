<?php
    ...
    class M_tr_model extends CI_Model{
        ...
        public function kupon($id=0,$user_id=0)

        {

            $this->db->trans_start();

            $cek=$this->getTableSingle("table_dis_coupon",array("id" => $id));



            // Kuponu kilitle ve durumunu kontrol et

            $query = $this->db->query('SELECT * FROM table_dis_coupon WHERE coupon_code = ? FOR UPDATE', array($cek->coupon_code));
            ...
            $this->db->where('coupon_code', $cek->coupon_code);

            $this->db->update('table_dis_coupon', array('status' => 2,"used_at" => date("Y-m-d H:i:s"),"used_user" => $user_id));
