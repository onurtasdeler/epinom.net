                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-9 col-12 mt-4 mt-md-0">
                                <div class="card shadow-sm">
                                    ...
                                    <div class="card-body">
                                        <div class="alert alert-info">Aşağıdaki formu doldurarak bakiyenizden ilgili tutar kadar çekim talebinde bulunabilirsiniz.
                                            <br>İncelemeler dahilinde, yetkililer tarafından, işlem sonuçlandırılacaktır.</div>
                                        <div class="alert alert-warning"><b>·</b> Para çekim talimatınızın banka hesabınıza gönderim süresi 24 saattir.<br><b>·</b> Haftasonu verilen para çekim talimatları <b>PAZARTESİ</b> günü gönderim sağlanır.</div>
                                        <?php
                                            if(isset($form_error)) :
                                        ?>
                                        ...
                                        <?php
                                            ...
                                            <?=form_open(current_url())?>
                                                <div class="form-group">
                                                    <label class="fw-bold">Banka Hesabı</label>
                                                    <?php

                                                        if (count($userBankAccounts) > 0) {
                                                            foreach($userBankAccounts as $_uba):
                                                    ?>
                                                        <div class="custom-control custom-radio">
                                                            <input type="radio" id="bankAccount<?=$_uba->id?>" name="bank_account" value="<?=$_uba->id?>" class="custom-control-input">
                                                            <label class="custom-control-label" for="bankAccount<?=$_uba->id?>"><?=$_uba->account_name . ' - ' . $_uba->full_name . ' - ' . $_uba->iban?></label>
                                                        </div>
                                                    <?php
                                                            endforeach;
                                                        } else {
                                                    ...
                                                    ?>
                                                        ...
                                                        <div class="text-danger">Hiç banka hesabınız yok. Banka hesabı eklemeniz gerekmektedir.</div>

                                                        <a href="<?=base_url('uye/banka-hesaplarim/olustur')?>" class="btn btn-primary btn-sm">Banka Hesabı Ekle</a>
                                                    ...
                                                    <div class="form-group mt-2">
                                                        ...
                                                        <label class="fw-bold">Çekmek İstediğiniz Tutar</label>

                                                        <input type="number" required class="form-control" name="amount" id="withdrawAmountInput" placeholder="Çekmek İstediğiniz Tutar" min="0" step="any" value="<?=set_value('amount')?>" oninput="this.value < 0 ? this.value = 0 : this.value = this.value">

                                                    </div>

                                                    <div class="form-group">

                                                        <label class="fw-bold">Çekilebilir Tutar</label>

                                                        <div class="h5 text-primary"><span><?=number_format($this->db->where('id=' . getActiveUser()->id)->get('users')->row()->balance,2)?></span>TL </div>

                                                    </div>

                                                    <div class="form-group">

                                                        <label class="fw-bold">Hesabınıza Aktarılacak Olan Net Ödeme Tutarı <span class="badge badge-primary shadow-sm">İşlem Ücreti: <?=($balanceWithdrawCommissionAmount > 0 ? $balanceWithdrawCommissionAmount . 'TL' : NULL)?></span></label>

                                                        <div class="h5 text-success"><span id="wonPrice">0.00</span>TL </div>

                                                    </div>

                                                    <div class="text-end">

                                                        <button class="btn btn-sm btn-primary" name="submitForm" value="<?=md5(uniqid())?>" type="submit">İşlemi Tamamla</button>
...
$(function(){
    ...
    function calculateThis(v){

        var value = parseFloat(v);

        if(value <= 0) {

            $('#wonPrice').html(formatMoney(parseFloat(0), 2, '.', ','));

            return;

        }

        var commissionAmount = parseFloat('<?=$balanceWithdrawCommissionAmount?>');

        if (commissionAmount>0) {

            if (value>parseFloat('<?=$balanceWithdrawCommissionAmount?>')) {

                $('#wonPrice').html(formatMoney(parseFloat(value-commissionAmount), 2, ',', ','));
...
?>
    ...
    calculateThis('<?=set_value('amount')?>');
...
?>
    ...
    $('#withdrawAmountInput').on('input', function(){
        ...
        calculateThis($(this).val());

    });

    $('#withdrawAmountInput').on('blur', function(){
