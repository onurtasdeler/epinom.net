                        ...
                        <div class="row">
                            <div class="col-lg-3 col-12">
                                <div class="card shadow-sm">
                                    ...
                                    <div class="card-body p-0">        
                                        ...
                                        <a href="<?=base_url("uye/guvenlik-ayarlari")?>" class="list-group-item list-group-item-action active border-left-0 border-right-0">
                                            Güvenlik Ayarları
                                        </a>
                            ...
                            <div class="col-lg-9 col-12 mt-4 mt-md-0">
                                <div class="card shadow-sm">
                                    ...
                                    ?>
                                    <?=form_open('uye/guvenlik-ayarlari')?>
                                        <div class="alert alert-info">
                                            Belirleyeceğiniz IP adresleri sistem tarafından izin verilen IP adresleri olacaktır. Farklı bir IP adresi ile giriş yapamazsınız.
                                        </div>
                                        <div class="form-group">
                                            <label class="form-label font-weight-bold">IP Adresi Kontrolü</label>
                                            <div class="custom-control custom-switch">
                                                <input type="checkbox" name="ip_control" <?=$user->ip_control == 1 ? 'checked' : NULL?> value="1" class="custom-control-input" id="customSwitch1">
                                                <label class="custom-control-label" for="customSwitch1">Kontrol Yapılmasın</label>
                                            </div>
                                        </div>
                                        <div id="ipAddressesParent" class="form-group"<?=$user->ip_control == 0 ? ' style="display:none;"' : NULL?>>
                                            <label class="form-label font-weight-bold">IP Adresleri</label>
                                            <textarea name="ip_addresses" rows="4" class="form-control"><?=!empty($user->ip_addresses) ? implode("\n", json_decode($user->ip_addresses)) : NULL?></textarea>
                                            <p class="help-block text-muted small">Her satırda bir IP adresi olacak şekilde IP adreslerinizi belirleyiniz.</p>
                                        </div>
                                        <div class="text-end mt-3">
                                            <button type="submit" name="submitForm" value="ok" class="btn btn-sm btn-primary px-5">
                                                Ayarları Kaydet
                                            </button>
                                        </div>
                                    <?=form_close()?>
...
$('#customSwitch1').on('click', function(){
    if ($('[for="customSwitch1"]').text() == 'Kontrol Yapılmasın'){
        $('[for="customSwitch1"]').text('Kontrol Yapılsın');
        $('#ipAddressesParent').fadeIn(200);
    } else {
        $('[for="customSwitch1"]').text('Kontrol Yapılmasın');
        $('#ipAddressesParent').fadeOut(200);
    }
});
</script>
