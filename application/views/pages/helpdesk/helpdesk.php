			...
			<div class="page d-flex flex-row flex-column-fluid">
                ...
                <div class="toolbar py-5 py-lg-5" id="kt_toolbar">
					...
					<div id="kt_toolbar_container" class="container-xxl d-flex flex-stack flex-wrap">
						...
						<div class="page-title d-flex flex-column me-3">
							<!--begin::Title-->
							<h1 class="d-flex text-dark fw-bolder my-1 fs-3">Destek</h1>
							<!--end::Title-->
							<!--begin::Breadcrumb-->
                        ...
                        <div class="d-flex align-items-center py-2 py-md-1">
                        	<!--begin::Button-->
							<a href="<?=base_url("destek/yeni-destek-talebi")?>" class="btn btn-primary fw-bolder">Destek Talebi Oluştur</a>
							<!--end::Button-->
						</div>
					</div>
					<!--end::Container-->
				...
				<div id="kt_content_container" class="d-flex flex-column-fluid align-items-start container-xxl">
					...
					<div class="content flex-row-fluid" id="kt_content">
                        <div class="row g-5">
                            <?php
                                if(isset($activeUserHelpDesks) && count($activeUserHelpDesks)>0){
                            ?>
                            <?php
                                foreach($activeUserHelpDesks as $helpDesk):
                            ?>
                            <div class="col-12">
                                <div class="d-flex card-rounded flex-wrap justify-content-between shadow-sm p-5 gap-5">
                                    <div class="ticket-header w-100 w-sm-auto text-center text-sm-start">
                                        <b><?=$helpDesk->title?></b><br/>
                                        <span><?=mb_substr(strip_tags($helpDesk->text),0,160) . "..."?></span>
                                    </div>
                                    <div class="ticket-action d-flex flex-wrap gap-5 w-100 w-sm-auto">
                                        <div class="ticket-date text-center w-100 w-sm-auto">
                                            <b>Oluşturulma Tarihi</b><br/>
                                            <span><?=date("d/m/Y H:i", strtotime($helpDesk->created_at))?></span>
                                        </div>
                                        <div class="ticket-actions w-100 w-sm-auto text-center text-sm-start">
                                            <a href="<?=base_url("destek/talep/" . $helpDesk->help_code)?>" class="btn btn-sm btn-primary">Detay</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <?php endforeach; ?>
                            <?php } else { ?>
                                <div class="alert alert-info mb-0">Hiç destek kaydınız bulunmuyor.</div>
                            <?php } ?>
                        </div>
                    </div>
                </div>
