					...
					<div class="content flex-row-fluid" id="kt_content">
                        <div class="row g-5">
                            <div class="col-xl-8 col-12">
                                <div class="card-rounded shadow-sm p-5 d-flex justify-content-center justify-content-sm-between align-items-center flex-wrap product-detail-card">
                                ...
                                $_modal_p_sp_fields = $this->db->where([
                                                            ...
                                                            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                                                                <div class="modal-content">
                                                                    <div class="modal-header">
                                                                        <h5 class="modal-title"><?=$product->product_name?> Satın Al</h5>
                                                                    ...
                                                                    <div class="modal-body">
                                                                        <div class="row">
                                                                            ...
                                                                            <?php
                                                                                if (!empty($product->special_fields_alert)) {
                                                                            ?>
                                                                                <div class="alert alert-warning"><i data-feather="alert-triangle" width="16" height="16"></i> <?=$product->special_fields_alert?></div>
                                                                            <?php
                                                                                }
                                                                            ?>
                                                                                <div id="productModalFormResponse<?=$product->id?>"></div>
                                                                                <div class="form-group" id="quantityP<?=md5($product->id)?>FormGroup">
                                                                                    <label class="form-label font-weight-bold">Adet</label>
                                                                                    <input type="number" step="1" min="1" name="quantity" data-qty-price="<?=getProductPrice($product)?>" data-target="#qtyXprice_specialfields<?=md5($product->id)?>" id="quantityP<?=md5($product->id)?>" placeholder="Adet" value="1" class="form-control w-25 specialfields_qty">
                                                                                </div>
                                                                                <div class="form-group">
                                                                                    <label class="form-label font-weight-bold">Fiyat</label>
                                                                                    <div id="qtyXprice_specialfields<?=md5($product->id)?>" class="text-primary font-weight-bold"><?=money_format(getProductPrice($product), 2)?> AZN</div>
                                                                                </div>
                                                                                <form action="javascript:void(0)" method="post" id="productModal<?=$product->id?>Form">
                                                                                    <?php
                                                                    ...
                                                                    <div class="modal-footer border-0 d-md-flex d-block text-md-right text-left">
                                                                        ...
                                                                        <button type="button" class="btn btn-primary add-to-cart-modal" data-response-area="#productModalFormResponse<?=$product->id?>" data-pname="<?=$product->product_name?>" data-input="#quantityP<?=md5($product->id)?>" data-pid="<?=md5($product->id) . "-" . $product->id?>" data-form="#productModal<?=$product->id?>Form">Sepete Ekle</button>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                    ...
                                    <ul class="nav nav-tabs nav-line-tabs nav-line-tabs-2x mb-5 fs-6 mt-5">
                                        <li class="nav-item">
                                            <a class="nav-link active" data-bs-toggle="tab" href="#nav-howtouse">Nasıl Kullanılır?</a>
                                        </li>
                                        <li class="nav-item">
                                    ...
                                    <div class="tab-content" id="nav-tabContent">
                                        <div class="tab-pane fade show active" id="nav-howtouse" role="tabpanel" aria-labelledby="nav-howtouse-tab">
                            ...
                            <div class="col-xl-4 col-12">
                                <div class="card shadow-sm">
                                    <div class="card-header">
                                        <h3 class="card-title">Benzer ürünler</h3>
                                    </div>
                                </div>
                                <?php foreach($other_products as $product): ?>
                                    
                                    <a href="<?= base_url('oyunlar/' . $category->category_url . '/' . $product->product_url . '/' . $product->id) ?>" class="card-rounded shadow-sm p-5 d-flex justify-content-center justify-content-sm-between align-items-center flex-wrap product-detail-card mt-3">
                                        <img src="<?=base_url('public/categories/' . productImage($product, $category))?>"  alt="<?=$product->product_name?>" class="rounded shadow-sm">
                                        <div class="d-flex h-100 flex-column justify-content-center align-items-center product-info">
                                            <b class="text-primary text-center fw-bold"><?= $product->product_name ?></b>
                                            <span class="text-muted text-center"><?= $product->product_value ?></span>
                                            <?php
                                                if(!empty($product->discount) && $product->discount>0 && ((!empty($product->discount_end_date) && (time()-strtotime($product->discount_end_date)) <= 0) || empty($product->discount_end_date))){
                                            ?>
                                        ...
                                        <div class="d-flex h-100 flex-column justify-content-center align-items-center align-items-sm-end">
                                            <div class="pricing fw-bold">
                                                ...
                                                <?php } ?>
                                                <span class="text-dotted <?=(!empty($product->discount) && $product->discount>0 && ((!empty($product->discount_end_date) && (time()-strtotime($product->discount_end_date)) <= 0) || empty($product->discount_end_date))) ? 'text-primary' : 'text-dark'?>" id="priceArea<?=$product->id?>">
                                                    <?=number_format(getProductPrice($product), 2)?> <small>AZN</small>
                                                </span>
                                            </div>
                                        </div>
