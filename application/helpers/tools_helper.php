...
function isTcOk($user_id) {
    $user = getUser($user_id);
    return !empty($user->tc_no);
}
...
function getLoggedInUser(){
    if (getActiveUser()) {
        return getUser(getActiveUser()->id);
    }
    return (object)[];
}
...
function websiteStatus(){
    $CI = &get_instance();
    if($CI->db->where('id=1')->get('config')->row()->site_status == 0){
        $CI->load->view('website_offline');
    }
}
...
function generateUserHash(){
    return md5(randomPassword(5));
}

function randomPassword($length, $alphabet = null){
    if($alphabet == null){
...
function _curl($url, $postArray = null){
	$ch = curl_init();
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        ...
        'ApiName: a732e1abd2259d637e0293df2fff5a20',
        'ApiKey: 146ffe88dba167b93112c0db073ef046',
    ]);
